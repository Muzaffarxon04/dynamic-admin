...
import AddCompanyModal from 'src/components/Modals/AddModal/AddOrder-modal';
...
const Page = () => {
 ...
 const dispatch = useDispatch();
   ...
   const [rowsPerPage, setRowsPerPage] = useState(pageCount || 5);
    const initalData = data["/review/all"]?.reviews;
        ...
        setFiltered(
          initalData.filter((user) => {
              if (searchValue == "") {
                ...
                user?.info?.toLowerCase().includes(searchValue.toString()?.toLowerCase()) ||
                user?.stars?.toString()?.toLowerCase().includes(searchValue.toString()?.toLowerCase())
              ) {
                return user;
              }
            })
